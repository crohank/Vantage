...
class GeminiService(LLMService):
    ...
    def invoke(self, prompt: str, temperature: float = 0.7, agent_name: str = "unknown", analysis_id: str = None, **kwargs) -> str:
        """
        ...
        Args:
            ...
            analysis_id: Analysis run ID (for telemetry)
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if "candidates" in result and len(result["candidates"]) > 0:
                        ...
                        prompt_tokens = usage.get("promptTokenCount", get_telemetry_db().estimate_tokens(prompt))
                        completion_tokens = usage.get("candidatesTokenCount", get_telemetry_db().estimate_tokens(text))
                        try:
                            get_telemetry_db().log_call(
                            ...
                            print(f"[Telemetry] Warning: failed to log call: {te}")
...
class HuggingFaceService(LLMService):
    ...
    def invoke(self, prompt: str, temperature: float = 0.3, agent_name: str = "unknown", analysis_id: str = None, **kwargs) -> str:
        """
        ...
        try:
            ...
            # Log telemetry (HF doesn't return token counts, so estimate)
            latency_ms = (time.time() - call_start) * 1000
            prompt_tokens = telemetry.estimate_tokens(prompt)
            ...
            try:
                ...
                # Log telemetry for fallback path
                latency_ms = (time.time() - call_start) * 1000
                try:
                    telemetry.log_call(
                        agent_name=agent_name,
                ...
                try:
                    ...
                    print(f"[Telemetry] Warning: failed to log error: {te}")
